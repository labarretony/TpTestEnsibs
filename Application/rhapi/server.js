...
var port = process.env.npm_package_config_port || 8086;
...
app.delete('/api/deleteall', function (req, res) {
    chaine = '{"data":[]}';
    fs.writeFileSync(path.join(__dirname, "data", "salarie.json"), chaine, "UTF-8");
    levelmessage = 'info';
    message = '';
    res.sendStatus(200)
});
...
app.get('/api/rechercher', function (req, res) {
    ...
    var listesalarie = JSON.parse(fs.readFileSync(path.join(__dirname, "data", "salarie.json"), "UTF-8"));
...
//ecoute du port
console.log("Fichiers présents dans data :", fs.readdirSync(path.join(__dirname, "data")));
app.listen(port);
